...
if "leveldb" not in leveldb_ok:
    ...
    vsa_data = vsa if isinstance(vsa, dict) else (vsa[0] if isinstance(vsa, list) and len(vsa) > 0 else {})
    check("L5a: VersionSet::LogAndApply 文件指向 version_set.cc",
          vsa_data.get("file", "").endswith("version_set.cc") if isinstance(vsa_data, dict) else False, True)
    check("L5b: VersionSet::LogAndApply 行号 ≈ 777",
          abs(vsa_data.get("line_start", 0) - 777) <= 2 if isinstance(vsa_data, dict) else False, True)
