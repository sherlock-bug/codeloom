...
impl Symbol {
    pub fn insert(&self, conn: &Connection) -> anyhow::Result<i64> {
        Ok(conn.query_row(
            "INSERT INTO symbols (repo,name,kind,content_hash,file_path,line_start,line_end,language,signature,parent_class,namespace) \
             ...
             RETURNING id",
            rusqlite::params![self.repo,self.name,self.kind,self.content_hash,self.file_path,self.line_start,self.line_end,self.language,self.signature,self.parent_class,self.namespace],
            |row| row.get(0),
        )?)
    }
}