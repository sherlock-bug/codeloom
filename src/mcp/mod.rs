...
pub async fn serve_stdio() -> anyhow::Result<()> {
    eprintln!("CodeLoom MCP Server v0.1.0 (stdio)");
...
fn tools_list(id: serde_json::Value) -> serde_json::Value {
    serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"tools":[
        ...
        {"name":"codeloom_status","description":"查看索引状态和统计信息。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"repo":{"type":"string"},"branch":{"type":"string"}}}},
        {"name":"codeloom_list_symbols","description":"按名称模糊搜索符号。优于grep：返回结构化结果。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"pattern":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["pattern"]}},
        {"name":"codeloom_get_definition","description":"获取符号完整定义（含源码、签名、文件路径、行号）。优先用此而非read_file。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"}},"required":["name"]}},
        {"name":"codeloom_get_call_graph","description":"分析函数调用关系：回答'X调用了哪些函数'(direction=callees)和'谁调用了X'(direction=callers)。当用户问某个函数/方法调用了什么或被谁调用时，必须优先用此工具——grep和文件搜索无法展示调用链层次关系。支持模糊名自动匹配。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"direction":{"type":"string","enum":["callers","callees"]},"max_depth":{"type":"integer","default":3}},"required":["name"]}},
        {"name":"codeloom_semantic_search","description":"自然语言语义搜索代码+文档。用中文描述意图即可（如'数据压缩怎么实现'），不依赖精确关键词。优于grep：理解语义。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"query":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":10}},"required":["query"]}},
        {"name":"codeloom_search","description":"全文搜索符号名和定义。优于grep：返回结构化结果。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"query":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["query"]}},
        {"name":"codeloom_overview","description":"仓库架构全貌：符号/边/文档按类型统计，Top引用。branch参数传当前git分支名","inputSchema":{"type":"object","properties":{"repo":{"type":"string"},"branch":{"type":"string"}}}},
...
fn overview(repo: &str, branch: &str) -> String {
    ...
    let total_syms: i64 = if branch.is_empty() { conn.query_row("SELECT COUNT(*) FROM symbols WHERE repo=?1", rusqlite::params![repo], |r| r.get(0)).unwrap_or(0) } else { conn.query_row("SELECT COUNT(*) FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2", rusqlite::params![repo, branch], |r| r.get(0)).unwrap_or(0) };
    ...
    if let Ok(mut stmt) = conn.prepare(if branch.is_empty() { "SELECT kind, COUNT(*) FROM symbols WHERE repo=?1 GROUP BY kind ORDER BY COUNT(*) DESC" } else { "SELECT kind, COUNT(*) FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2 GROUP BY kind ORDER BY COUNT(*) DESC" }) {
...
fn status(repo: &str, branch: &str) -> String {
    ...
    let syms: i64 = if branch.is_empty() { conn.query_row("SELECT COUNT(*) FROM symbols WHERE repo=?1", rusqlite::params![repo], |r| r.get(0)).unwrap_or(0) } else { conn.query_row("SELECT COUNT(*) FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2", rusqlite::params![repo, branch], |r| r.get(0)).unwrap_or(0) };
...
fn get_call_graph(name: &str, repo: &str, branch: &str, direction: &str, max_depth: usize) -> String {
    ...
    let sym_ids: Vec<i64> = match conn.prepare(if branch.is_empty() { "SELECT id FROM symbols WHERE repo=?1 AND name=?2" } else { "SELECT s.id FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2 AND s.name=?3" }) {
    ...
    if sym_ids.is_empty() {
        ...
        let like = format!("%{}%", name);
        let similar: Vec<(i64, String)> = match conn.prepare(if branch.is_empty() { "SELECT id, name FROM symbols WHERE repo=?1 AND name LIKE ?2 LIMIT 10" } else { "SELECT s.id, s.name FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2 AND s.name LIKE ?3 LIMIT 10" }) {
    ...
    if let Ok(mut stmt) = conn.prepare(&query) {
        if let Ok(rows) = stmt.query_map([], |r| Ok((r.get::<_,i64>(0)?, r.get::<_,String>(1)?))) {
            for row in rows.flatten() {
                ...
                let (name_sql, name_params): (&str, Vec<&dyn rusqlite::types::ToSql>) = if branch.is_empty() {
                    ("SELECT name FROM symbols WHERE id=?1", vec![&other_id as &dyn rusqlite::types::ToSql])
                } else {
                    ("SELECT s.name FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE s.id=?1 AND b.branch_name=?2", 
                     vec![&other_id as &dyn rusqlite::types::ToSql, &branch as &dyn rusqlite::types::ToSql])
                };
                let other_name = conn.query_row(name_sql, name_params.as_slice(), |r| r.get::<_,String>(0)).unwrap_or_default();
...
fn semantic_search(query: &str, repo: &str, branch: &str, limit: usize) -> String {
    ...
    // Search symbols
    if let Ok(mut stmt) = conn.prepare(if branch.is_empty() { "SELECT id,name,kind,definition FROM symbols WHERE repo=?1" } else { "SELECT s.id,s.name,s.kind,s.definition FROM symbols s JOIN branches b ON s.id=b.symbol_id WHERE b.repo=?1 AND b.branch_name=?2" }) {
        if let Ok(rows) = stmt.query_map(rusqlite::params![repo], |r| {
            Ok((r.get::<_,i64>(0)?, r.get::<_,String>(1)?, r.get::<_,String>(2)?, r.get::<_,String>(3)?))
        }) {
            for row in rows.flatten() {
