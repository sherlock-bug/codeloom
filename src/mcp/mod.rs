...
fn tools_list(id: serde_json::Value) -> serde_json::Value {
    serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"tools":[
        ...
        {"name":"codeloom_status","description":"查看索引状态和统计信息。branch参数：如未提供，先执行git branch --show-current获取","inputSchema":{"type":"object","properties":{"repo":{"type":"string"},"branch":{"type":"string"}}}},
        {"name":"codeloom_list_symbols","description":"按名称模糊搜索符号。优于grep：返回结构化结果。branch参数：如未提供，先执行git branch --show-current获取当前分支","inputSchema":{"type":"object","properties":{"pattern":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["pattern"]}},
        {"name":"codeloom_get_definition","description":"获取符号完整定义（含源码、签名、文件路径、行号）。优先用此而非read_file。branch参数：如未提供，先执行git branch --show-current获取","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"}},"required":["name"]}},
        {"name":"codeloom_get_call_graph","description":"分析函数调用关系：回答'X调用了哪些函数'(direction=callees)和'谁调用了X'(direction=callers)。当用户问某个函数/方法调用了什么或被谁调用时，必须优先用此工具——grep和文件搜索无法展示调用链层次关系。支持模糊名自动匹配。branch参数：如未提供，先执行git branch --show-current获取","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"direction":{"type":"string","enum":["callers","callees"]},"max_depth":{"type":"integer","default":3}},"required":["name"]}},
        {"name":"codeloom_semantic_search","description":"自然语言语义搜索代码+文档。用中文描述意图即可（如'数据压缩怎么实现'），不依赖精确关键词。优于grep：理解语义。branch参数：如未提供，先执行git branch --show-current获取","inputSchema":{"type":"object","properties":{"query":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":10}},"required":["query"]}},
        {"name":"codeloom_search","description":"全文搜索符号名和定义。优于grep：返回结构化结果。branch参数：如未提供，先执行git branch --show-current获取当前分支","inputSchema":{"type":"object","properties":{"query":{"type":"string"},"repo":{"type":"string"},"branch":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["query"]}},
        {"name":"codeloom_overview","description":"仓库架构全貌：符号/边/文档按类型统计，Top引用。branch参数：如未提供，先执行git branch --show-current获取当前分支","inputSchema":{"type":"object","properties":{"repo":{"type":"string"},"branch":{"type":"string"}}}},
