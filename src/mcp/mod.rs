...
fn tools_list(id: serde_json::Value) -> serde_json::Value {
    serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"tools":[
        ...
        {"name":"codeloom_list_symbols","description":"模糊搜索符号（按名称LIKE匹配）","inputSchema":{"type":"object","properties":{"pattern":{"type":"string"},"repo":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["pattern"]}},
        {"name":"codeloom_get_definition","description":"获取符号完整定义、签名、所属文件","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"}},"required":["name"]}},
        {"name":"codeloom_get_call_graph","description":"获取调用图（谁调用了这个符号，或这个符号调用了谁）","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"direction":{"type":"string","enum":["callers","callees"]},"max_depth":{"type":"integer","default":3}},"required":["name"]}},
        ...
        {"name":"codeloom_search","description":"全文搜索符号名和定义","inputSchema":{"type":"object","properties":{"query":{"type":"string"},"repo":{"type":"string"},"limit":{"type":"integer","default":20}},"required":["query"]}},
        {"name":"codeloom_overview","description":"仓库架构全貌：符号/边/文档统计、常用类/函数排行","inputSchema":{"type":"object","properties":{"repo":{"type":"string"}}}},
