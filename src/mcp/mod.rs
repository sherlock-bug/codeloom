...
fn tools_list(id: serde_json::Value) -> serde_json::Value {
    serde_json::json!({"jsonrpc":"2.0","id":id,"result":{"tools":[
        ...
        {"name":"codeloom_get_call_graph","description":"分析函数调用关系：回答'X调用了哪些函数'(direction=callees)和'谁调用了X'(direction=callers)。当用户问某个函数/方法调用了什么或被谁调用时，必须优先用此工具——grep和文件搜索无法展示调用链层次关系。支持模糊名自动匹配，max_depth控制递归深度","inputSchema":{"type":"object","properties":{"name":{"type":"string"},"repo":{"type":"string"},"direction":{"type":"string","enum":["callers","callees"]},"max_depth":{"type":"integer","default":3}},"required":["name"]}},
