...
def get_node_file(node):
    ...
    if file:
        return file
    # Header declarations: loc.file is empty — fall back to includedFrom.file.
    # includedFrom.file points to the .cc file that #included the header.
    # This is not the header's own path, but it lets is_system() check
    # whether the includer is within the project, preserving project header
    # declarations while external/system headers get filtered downstream.
...
def is_system(node, project_root, fallback_file=None):
    ...
    import json
    json.dump({"kind":"WRITE_EVENT","file":file,"prev":node.get("previousDecl",""),"name":node.get("name",""),"kind":node.get("kind","")}, open("/tmp/filter_debug.ndjson","a"))
    json.dump({"kind":"WRITE_EVENT"}, open("/tmp/filter_debug.ndjson","a"))
