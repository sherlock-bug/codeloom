...
## Purpose
CodeLoom MCP 图分析工具层——为所有图分析类 MCP 工具（call_graph、inheritance_tree、neighbor_graph、impact_analysis、path_analysis、inspect_symbol）定义共享的查询规范，包括分支隔离行为。本域描述这些工具在查询 `edges` 表时的统一过滤约束和参数传递约定。
...
### Requirement: 继承树和虚函数覆写边查询按分支过滤
`inheritance_tree` 工具中的 `build_tree` 和 `get_overrides` 嵌套函数 SHALL 将外层解析的 `branch_id` 通过参数传入，而非硬编码为 0 回退值。

#### Scenario: 多分支继承树独立
- GIVEN 同一仓库的两个分支对类继承关系有不同定义
- WHEN 查询其中一个分支的继承树
- THEN 返回的父类/子类关系和虚函数覆写列表 SHALL 仅包含该分支的数据
