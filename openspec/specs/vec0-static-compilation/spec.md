# vec0-static-compilation

## Purpose
CodeLoom vec0-static-compilation 功能域。本规范描述此功能的需求和行为。

## Purpose
CodeLoom vec0 向量扩展的静态编译模块。将 sqlite-vec C 源码直接编译进 Rust 二进制，消除运行时外部动态库 .so 的依赖，实现真正的单文件部署和 musl 静态链接支持。
